 ...
 *   - Native Responses API inference with SSE streaming,
...
export class ComposeSDK {
    ...
    constructor(options: ComposeSDKOptions = {}) {
        ...
        this.x402 = new X402Resource(
            ...
            (input, init) => this.fetch(input, { ...(init ?? {}), paymentMode: "x402" } as RequestInit & CallOptions),
        );
     ...
     */
    async fetch(input: RequestInfo | URL, init: RequestInit & CallOptions = {}): Promise<Response> {
        const {
            x402MaxAmountWei,
            paymentSignature,
            paymentMode = "auto",

...
function delay(ms: number, signal?: AbortSignal): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        const onAbort = () => {
            clearTimeout(timer);
            reject(new TimeoutError({ message: "request aborted" }));
        };
        if (signal) {
            if (signal.aborted) {
                clearTimeout(timer);
...
function parseErrorBody(body: unknown): { code: ErrorCode; message: string; details?: Record<string, unknown> } {
    if (typeof body === "string") {
        const message = body.trim().slice(0, 1_000);
        return { code: "internal_error" as ErrorCode, message: message || "Request failed" };
    }
    if (body && typeof body === "object") {
        const envelope = body as ErrorEnvelope;
        if (envelope.error && typeof envelope.error === "object") {
            return {
                code: (envelope.error.code as ErrorCode) || "internal_error",
        ...
        if (pr.x402Version === 2 && Array.isArray(pr.accepts)) {
            ...
            return { code: "payment_required" as ErrorCode, message: rawError };
        }
        // Legacy shapes
        const legacy = body as Record<string, unknown>;
        if (typeof legacy.error === "string") {
            return { code: (typeof legacy.code === "string" ? legacy.code : "internal_error") as ErrorCode, message: legacy.error };
        }
        if (typeof legacy.message === "string") {
            return { code: "internal_error" as ErrorCode, message: legacy.message };
        }
    }
    return { code: "internal_error" as ErrorCode, message: "Request failed" };
}
        ...
        while (true) {
            ...
            try {
                ...
                if (isAbort && config.signal?.aborted) {
                    throw new TimeoutError({ message: "request aborted" });
                }
                if (isAbort) {
            ...
            const parsed = response.status === 429 && parsedError.code === "internal_error"
                ? { ...parsedError, code: "rate_limited" as ErrorCode }
                : parsedError;

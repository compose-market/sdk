 ...
 * NO secrets, NO ValKey, NO Thirdweb, NO environment variables.
...
const SDK_REPO_ROOT = new URL("../../", import.meta.url);
const SHARED_ROOT = new URL("shared/", SDK_REPO_ROOT);
...
test("generated x402 compose-keys client does not repeat the resource namespace", async () => {
    ...
    assert.doesNotMatch(source, /keysKeys|async keys(?:Create|List|Get|Revoke)\(/);
...
test("generated function exports do not repeat the same namespace twice", async () => {
    const generatedFiles = await files(new URL("generated/", SDK_PACKAGE_ROOT));
    const funcFiles = generatedFiles.filter((file) => /\/src\/funcs\/[^/]+\.ts$/.test(file.pathname));
    const duplicateFuncFiles = funcFiles
        .map((file) => file.pathname.split("/").at(-1)!.replace(/\.ts$/, ""))
        .filter((name) => {
            const [first, second] = name.split("-");
            return Boolean(first && second && first === second);
        });

    assert.deepEqual(duplicateFuncFiles, []);

    const sdkFiles = generatedFiles.filter((file) => /\/src\/sdk\/[^/]+\.ts$/.test(file.pathname));
    for (const file of sdkFiles) {
        const source = await readFile(file, "utf-8");
        assert.doesNotMatch(source, /from "\.\.\/funcs\/([a-z0-9]+)-\1-/);
    }
});

test("manowar connector contracts use the direct production connectors Worker routes", async () => {
    const spec = await readFile(new URL("specs/manowar.openapi.yaml", SHARED_ROOT), "utf-8");
    const generated = await Promise.all([
        "generated/manowar/src/funcs/connectors-mcp-search.ts",
        "generated/manowar/src/funcs/connectors-mcp-spawn-get.ts",
        "generated/manowar/src/funcs/connectors-mcp-actions-list.ts",
        "generated/manowar/src/funcs/connectors-mcp-action-execute.ts",
        "generated/manowar/src/funcs/connectors-mcp-inspect.ts",
        "generated/manowar/src/funcs/connectors-onchain-list.ts",
        "generated/manowar/src/funcs/connectors-onchain-get.ts",
        "generated/manowar/src/funcs/connectors-onchain-action-get.ts",
        "generated/manowar/src/funcs/connectors-onchain-action-execute.ts",
    ].map((path) => readFile(new URL(path, SDK_PACKAGE_ROOT), "utf-8")));
    const generatedSource = generated.join("\n");

    assert.match(spec, /url: https:\/\/connectors\.compose\.market/);
    assert.match(spec, /"\/mcps\/search":/);
    assert.match(spec, /"\/mcps\/\{serverId\}\/spawn":/);
    assert.match(spec, /"\/mcps\/\{serverId\}\/execute\/\{command\}":/);
    assert.match(spec, /"\/mcps\/\{serverId\}\/inspect":/);
    assert.match(spec, /"\/onchain\/\{connectorId\}":/);
    assert.match(spec, /"\/onchain\/\{connectorId\}\/execute\/\{command\}":/);
    assert.doesNotMatch(spec, /"\/api\/(?:mcps|onchain)(?:\/|")/);
    assert.doesNotMatch(generatedSource, /\/onchain\/plugins/);
    assert.doesNotMatch(generatedSource, /pathToFunc\("\/api\/(?:mcps|onchain)/);
    assert.match(generatedSource, /pathToFunc\("\/mcps/);
    assert.match(generatedSource, /pathToFunc\("\/onchain/);
    assert.match(generatedSource, /ServerConnectors/);
});

test("public SDK inference contracts expose Responses-only model execution", async () => {
    const inferenceSpec = await readFile(new URL("specs/inference.openapi.yaml", SHARED_ROOT), "utf-8");
    ...
    for (const path of forbiddenPaths) {
        assert.doesNotMatch(inferenceSpec, new RegExp(`"${path.replace(/\//g, "\\/")}`));
    }
    assert.match(inferenceSpec, /"\/v1\/responses":/);

    const roots = [
        new URL("src/resources/inference.ts", SDK_PACKAGE_ROOT),
        new URL("specs/inference.openapi.yaml", SHARED_ROOT),
        new URL("generated/inference/src/", SDK_PACKAGE_ROOT),
    ];

    for (const root of roots) {
        const candidates = root.pathname.endsWith("/") ? await files(root) : [root];
        for (const file of candidates) {
            const source = await readFile(file, "utf-8");
            const relative = fileURLToPath(file).replace(fileURLToPath(SDK_PACKAGE_ROOT), "");
            assert.doesNotMatch(source, /\/v1\/chat\/completions/, `${relative} exposes obsolete chat completions route`);
            assert.doesNotMatch(source, /chat[_-]completions/i, `${relative} exposes obsolete chat completions operation`);
            assert.doesNotMatch(source, /\/v1\/embeddings/, `${relative} exposes obsolete embeddings route`);
            assert.doesNotMatch(source, /\/v1\/images\//, `${relative} exposes obsolete images route`);
            assert.doesNotMatch(source, /\/v1\/audio\//, `${relative} exposes obsolete audio route`);
            assert.doesNotMatch(source, /\/v1\/videos(?:\/|")/, `${relative} exposes obsolete videos route`);
            assert.doesNotMatch(source, /\/api\/inference/, `${relative} exposes obsolete legacy inference route`);
        }
    }
});

test("public SDK contracts do not expose Mesh product routes", async () => {
    const roots = [
        new URL("specs/", SHARED_ROOT),
        new URL("generated/manowar/src/", SDK_PACKAGE_ROOT),
    ];
    ...
    for (const root of roots) {
        for (const file of await files(root)) {
            const source = await readFile(file, "utf-8");
            const relative = fileURLToPath(file).replace(fileURLToPath(SDK_PACKAGE_ROOT), "");
            assert.doesNotMatch(source, /\/mesh\//, `${relative} exposes Mesh product route`);
            assert.doesNotMatch(source, /x-local-runtime-token/, `${relative} exposes Mesh local runtime token`);
            assert.doesNotMatch(source, /\bmesh[A-Z_ -]/i, `${relative} exposes Mesh product namespace`);
        }
    }
});
...
test("package source excludes compatibility routes and unstable analytics resources", async () => {
    const roots = [
        new URL("src/", SDK_PACKAGE_ROOT),
        new URL("specs/", SHARED_ROOT),
        new URL("generated/", SDK_PACKAGE_ROOT),
        new URL("tests/", SDK_PACKAGE_ROOT),
    ];
    const deny = [
        ...
        /Compose(?:CallOptions|Completion|Error|ConnectionError|TimeoutError|PaymentMode|ErrorCode|ErrorEnvelope)/,
    ];
    ...
    for (const root of roots) {
        for (const file of await files(root)) {
            ...
            const relative = fileURLToPath(file).replace(fileURLToPath(SDK_PACKAGE_ROOT), "");
            if (relative.includes("/hooks/types.") || relative.includes("/hooks/index.")) continue;
...
test("handwritten SDK exposes Responses-only inference endpoints without planner or internal services", () => {
    const sdk = new ComposeSDK({ baseUrl: "https://api.example.test" });

    assert.equal(typeof sdk.inference.responses.create, "function");
    assert.equal("chat" in sdk.inference, false);
    assert.equal("embeddings" in sdk.inference, false);
    assert.equal("images" in sdk.inference, false);
    assert.equal("audio" in sdk.inference, false);
    assert.equal("videos" in sdk.inference, false);
...
test("responses request contract does not expose provider overrides", async () => {
    const spec = await readFile(new URL("specs/inference.openapi.yaml", SHARED_ROOT), "utf-8");
    const requestSchemas = ["ResponsesCreateRequest"];
...
test("memory public contract uses scope, never mode, for global/local memory", async () => {
    const roots = [
        new URL("src/resources/memory.ts", SDK_PACKAGE_ROOT),
        new URL("src/types/index.ts", SDK_PACKAGE_ROOT),
        new URL("specs/memory.openapi.yaml", SHARED_ROOT),
        new URL("generated/memory/src/", SDK_PACKAGE_ROOT),
    ];
    ...
    for (const root of roots) {
        ...
        for (const file of candidates) {
            const source = await readFile(file, "utf-8");
            const relative = fileURLToPath(file).replace(fileURLToPath(SDK_PACKAGE_ROOT), "");
            assert.doesNotMatch(source, /\bmode\??:\s*["']global["']\s*\|\s*["']local["']/, `${relative} exposes memory mode alias`);
            assert.doesNotMatch(source, /\bmode:\s*\n\s*type:\s*string\s*\n\s*enum:\s*\n\s*-\s*global\s*\n\s*-\s*local/, `${relative} exposes memory mode schema`);
        }
    }
});
...
test("memory.context assembles pre-turn context through the agent-first memory endpoint", async () => {
    const server = await startMockServer((req, res) => {
        ...
        assert.equal(body.haiId, "hai_test_1");
    ...
    try {
        ...
        const callsBeforeValidation = server.calls.length;
        assert.throws(
            () => sdk.memory.context({ agentWallet: "0xagent", query: "bad", mode: "local" } as any),
            (err: unknown) => err instanceof BadRequestError,
        );
        assert.throws(
            () => sdk.memory.context({ agentWallet: "0xagent", query: "bad", scope: "local" }),
            (err: unknown) => err instanceof BadRequestError,
        );
        assert.equal(server.calls.length, callsBeforeValidation);
    } finally {
        await server.close();
    }
});
...
test("feedback.submit retries transient 429 responses and preserves context", async () => {
    ...
    const server = await startMockServer((req, res) => {
        ...
        const body = JSON.parse(req.body);
        assert.equal(body.context.runId, "run_feedback_retry");
...
test("keys.create surfaces insufficient_balance 402 as PaymentRequiredError (budget_exhausted is its own subclass)", async () => {
    const server = await startMockServer((_req, res) => {
        sendJson(res, 402, {
            error: {
    ...
    try {
        ...
        assert.ok(caught instanceof PaymentRequiredError);
        const e = caught as PaymentRequiredError;
        ...
        if (req.path === "/api/local/link-token/redeem") {
            ...
            sendJson(res, 200, {
                ...
                context: {
                    ...
                    userAddress: WALLET,
                    chainId: 43113,
                    key: { keyId: "key_1", token: "compose-local", expiresAt: 1_800_000_000_000 },
...
test("x402 PAYMENT-REQUIRED header decoders round-trip", () => {
    ...
    const original = {
        ...
        resource: { url: "https://api.compose.market/v1/responses", description: "test", mimeType: "application/json" },
        accepts: [{
            scheme: "upto",
            network: "eip155:43114",
            amount: "250000",
...
test("inference auto-negotiates raw x402 when no Compose Key is present", async () => {
    ...
    const server = await startMockServer((req, res) => {
        ...
        sendJson(res, 200, {
            ...
            status: "completed",
            model: "gpt-4.1-mini",
            output: [{ type: "output_text", role: "assistant", text: "paid" }],
    ...
    try {
        const sdk = new ComposeSDK({
            ...
            x402Signer: (challenge) => {
                ...
                assert.equal(challenge.path, "/v1/responses");
        ...
        assert.equal(result.data.id, "resp-x402");
        assert.equal(result.data.output[0].text, "paid");
...
test("sdk.fetch and sdk.x402.fetch negotiate x402 v2 PAYMENT-REQUIRED challenges", async () => {
    const paymentRequired = {
        x402Version: 2 as const,
        resource: { url: "http://unused/agent/0xabc/stream", description: "Agent stream" },
        accepts: [{
            scheme: "upto",
            network: "eip155:43113",
            amount: "1000000",
    ...
    try {
        ...
        const first = await sdk.fetch("/agent/0xabc/stream", {
            method: "POST",
        ...
        const second = await sdk.x402.fetch("/agent/0xabc/stream", {
            method: "POST",
...
test("createX402EvmSigner creates a real v2 upto payment payload", async () => {
    ...
    const payload = await signer({
        paymentRequired: {
            x402Version: 2,
            resource: { url: "https://api.compose.market/agent/0xabc/stream" },
            accepts: [{
                scheme: "upto",
                network: "eip155:43113",
                amount: "1000000",
        ...
        method: "POST",
        path: "/agent/0xabc/stream",
        url: "https://api.compose.market/agent/0xabc/stream",
        userAddress: WALLET,
        chainId: 43113,
        maxAmountWei: "1000000",
    });
    ...
    assert.equal(payload.resource?.url, "https://api.compose.market/agent/0xabc/stream");
});
...
test("paymentMode x402 suppresses an instance Compose Key", async () => {
    ...
    try {
        ...
        assert.equal(result.data.id, "resp-manual-x402");
    } finally {
        await server.close();
    }
});
...
test("inference.responses.create returns typed data + receipt from header and JSON mirror", async () => {
    ...
    const server = await startMockServer((req, res) => {
        ...
        res.end(JSON.stringify({
            ...
            status: "completed",
            model: "gpt-4.1-mini",
            output: [{ type: "output_text", role: "assistant", text: "Hi!" }],
    ...
    try {
        ...
        const { data, receipt, requestId } = await sdk.inference.responses.create({
            model: "gpt-4.1-mini",
            input: "Say hi.",
        });
        assert.equal(data.id, "resp-1");
        assert.equal(data.output[0].text, "Hi!");
...
test("inference.responses.create forwards universal attachments without capability gating", async () => {
    const server = await startMockServer((req, res) => {
        assert.equal(req.method, "POST");
        assert.equal(req.path, "/v1/responses");
        const body = JSON.parse(req.body);
    ...
    try {
        ...
        assert.equal(result.data.id, "resp-attachments");
    } finally {
        await server.close();
    }
});

test("inference.responses.create forwards rich native inference fields unchanged", async () => {
    const server = await startMockServer((req, res) => {
        assert.equal(req.method, "POST");
        assert.equal(req.path, "/v1/responses");
        const body = JSON.parse(req.body);
        ...
        sendJson(res, 200, {
            ...
            model: "gpt-5.5",
            output: [
                { type: "reasoning", role: "assistant", text: "checked inputs" },
                { type: "output_text", role: "assistant", text: "ok" },
            ],
            usage: { input_tokens: 1, output_tokens: 1, total_tokens: 2 },
        });
    });
    try {
        ...
        const result = await sdk.inference.responses.create({
            model: "gpt-5.5",
            input: [{ role: "developer", content: "Return JSON." }, { role: "user", content: "Deploy" }],
            response_format: {
                type: "json_schema",
                json_schema: {
                    name: "deploy_result",
        ...
        assert.equal(result.data.id, "resp-rich-native");
        assert.equal(result.data.output[0].text, "checked inputs");
    } finally {
        await server.close();
    }
});
...
test("inference.responses.create rejects stream: true; caller must use .stream()", async () => {
    ...
    try {
        ...
        await assert.rejects(
            () => sdk.inference.responses.create({
                model: "gpt-4.1-mini",
                input: "hi",
                stream: true,
            }),
...
test("error envelope {error:{code,message}} maps to typed error classes per HTTP status", async () => {
    const cases: Array<{ status: number; code: string; expected: new (...a: unknown[]) => Error }> = [
        ...
        { status: 500, code: "internal_error", expected: APIError },
    ];

 ...
 *   - `Completion<T>` now carries `budget` + `sessionInvalidReason`.
...
test("Completion carries budget + sessionInvalidReason from response headers", async () => {
...
test("sdk.events emits budget + receipt on every billable response", async () => {
    ...
    const server = await startMockServer((_req, res) => {
        ...
        res.end(JSON.stringify({
            id: "resp-1", object: "response", created_at: 1, status: "completed", model: "gpt-4.1-mini",

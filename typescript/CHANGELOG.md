...
## 0.8.0 — Package restructuring, tool events, channels OpenAPI, x402 skill

### Highlights

- Eliminated duplicated code between SDK and core: `http.ts`, `storage.ts`, `streaming/budget.ts`, `streaming/receipt.ts` are now thin re-exports from `@compose-market/core`. The SDK no longer ships its own copies of `HttpClient`, `extractSessionBudgetFromResponse`, `decodeReceiptHeader`, `extractReceiptFromResponse`, `parseReceiptEvent`, `createMemoryStorage`, `resolveStorage`, or `buildTokenStorageKey`.
- Consolidated base wire types: `ErrorCode`, `PaymentRequired`, `PaymentPayload`, `Receipt`, `SessionBudgetSnapshot`, `SessionEvent`, `Alert`, and all other shared types are now re-exported from `@compose-market/core` instead of re-declared in the SDK.
- Added `tools` module with readable, productized tool event types: `ModelToolEvent`, `ConnectorToolEvent`, `AgentToolEvent`, `SearchToolEvent`, `CatalogToolEvent`, plus `parseToolEvent()` helper for parsing `tool_start`/`tool_end` SSE events.
- Added `commands` types re-export from core: `Command`, `NativeAction`, `GoalState`, `PlanState`, `TaskRecord`, `ActionResult`, etc.
- Added `msm` types re-export from core: `Mode`, `Scope`, `Stage`, `Classification`, `Snapshot`, `Route`, `Action`, `Transition`, `Observation`, etc.
- Created `channels.openapi.yaml` — 5th Speakeasy source covering the channels service (WhatsApp, Telegram, Slack, Discord linking/status/disconnect).
- Generated `@compose-market/sdk/channels` subpath from the new OpenAPI spec.
- Rewrote `a2a.arazzo.yaml` with complete x402 payment documentation covering all three EVM schemes (exact, upto, batch-settlement), the `PAYMENT-SIGNATURE` header format, channel recovery requirements, and SSE stream parsing.
- Created `commands.arazzo.yaml` with workflows for all 8 slash commands.
- Backported `combineSignals` abort listener leak fix to core (now returns `{ signal, cleanup }`).
- Core's `streaming/` module renamed to `sse/` for clarity.

### Breaking Changes

- `@compose-market/core` dependency bumped to `^0.2.0`
- Core's `streaming` subpath exports renamed to `sse` (e.g., `@compose-market/core/streaming/budget` → `@compose-market/core/sse/budget`)
- SDK no longer ships local copies of `http.ts`, `storage.ts`, `streaming/` — all re-exported from core
- Base wire types in `sdk/src/types/index.ts` are now re-exported from core, not re-declared

### New Exports

- `import { tools, parseToolEvent } from "@compose-market/sdk"` — tool event types and parser
- `import { commands, msm } from "@compose-market/sdk"` — command and state machine types
- `import { Channels } from "@compose-market/sdk/channels"` — generated channels SDK

### Tests

- `npm run typecheck` — clean
- `npm test` — 92/92 passing
- `cd packages/core && npm test` — 7/7 passing

## 0.7.13 — Production channel service split

### Highlights

- Added `channelsUrl` to `ComposeSDKOptions`, defaulting to `https://services.compose.market`, so `sdk.channels` targets the public channel service while API resources continue using `baseUrl`.
- Kept `sdk.permissions` and `sdk.accounts` on direct API routes (`/permissions`, `/accounts`) with `agentWallet` required everywhere relevant.
- Hardened `sdk.channels` as a transport-control client only: helpers call `/channels/*` on the channel service and strip Compose Key, x402, session, run, and payment headers even when the SDK is configured with API auth.
- Replaced stale legacy documentation and contract coverage with the production public resources: `sdk.permissions`, `sdk.accounts`, and `sdk.channels`.
- Documented Discord `mode` / guild `privacy`, Slack DM/workspace behavior, and the global session-renewal link behavior for channel integrations.
- Preserved the 0.6.97 inference stance: `sdk.inference.responses.stream()` remains the universal model stream path, with no SDK-side provider routing, planner/runner layer, `/external`, metrics, avatar/banner, or runtime-native additions.

### Tests

- `npm run typecheck`
- `node --import tsx --test tests/sdk.contract.test.ts --test-name-pattern "permissions and accounts|channels helpers"` verifies direct API resources plus `channelsUrl` routing and channel header stripping.
...
- Replaced raw permission/account/channel and workflow stop calls in `web/` with existing typed SDK resources.
...
- Added direct permission, account, toolkit/action discovery, action execution, and channel binding helpers.
...
- Added hermetic contract coverage for x402 payment intent prepare/settle/abort, model metering, pricing, public directory routes, Agentverse query forwarding, system routes, local routes, permission/account/channel resources, dispenser, settlement status, and package exclusion guards.
...
- Added explicit inference payment modes:
  ...
  - `key` disables raw x402 fallback.
...
- `EventMap` has new entries (`toolCallStart`, `toolCallEnd`, `agentStreamStart`, `agentStreamEnd`, `workflowStreamStart`, `workflowStreamEnd`). Consumers relying on the union of event names will widen automatically.
...
- **Live session budget on every response.** `Completion<T>` grows `budget` and `sessionInvalidReason` fields sourced from the `x-session-budget-*` / `x-session-invalid` response headers. No more side-channel fetches to `/api/session` to render a session indicator.
...
- `CallOptions.walletAddress` renamed to `CallOptions.userAddress`.
- `Completion<T>` now includes `budget` and `sessionInvalidReason`. Existing consumers that destructured `{ data, receipt, requestId, response }` are unaffected; consumers that did `const completion: Completion<T> = ...` and then structural-assigned may need to accept the new fields.
...
- **Typed cost receipts** — every billable call returns `{ data, receipt, requestId, response }`; streams emit a terminal `event: receipt` frame; non-streaming calls carry `X-Receipt` header + `receipt` JSON field.
...
- **x402 v2 first-class** — facilitator `supported`/`chains`/`verify`/`settle` + typed decoders for `PAYMENT-REQUIRED`, `PAYMENT-RESPONSE`, `X-Receipt`. New `sdk.x402.facilitator.chains()` enumerates live chains + USDC contract metadata.
- **Typed error tree** — `Error` → `APIError` → `BadRequestError` / `AuthenticationError` / `PermissionDeniedError` / `NotFoundError` / `ConflictError` / `UnprocessableEntityError` / `RateLimitError` / `InternalServerError` / `PaymentRequiredError` / `BudgetExhaustedError`. Every error carries `code`, `status`, `details`, `requestId`, and (for 402) `paymentRequired`.
- **Webhook verification** — HMAC-SHA256, Stripe-style `X-Signature: t=...,v1=...` header, constant-time comparison.
...
- `X-Request-Id` is set on every response; `X-Receipt` on billable responses.
- SSE streams emit named `receipt`, `compose.error`, `compose.video.status` events.

...
import { ComposeMarketDefaultError } from "../models/errors/compose-market-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | ComposeMarketDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<
        T,
        E | ComposeMarketDefaultError | ResponseValidationError
      >,
      raw: unknown,
    ]
  > {
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ComposeMarketDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ComposeMarketDefaultError("API error occurred", {
          request,
          response,
          body,
        }),

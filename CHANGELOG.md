...
- **Live session budget on every response.** `ComposeCompletion<T>` grows `budget` and `sessionInvalidReason` fields sourced from the `x-session-budget-*` / `x-session-invalid` response headers. No more side-channel fetches to `/api/session` to render a session indicator.
